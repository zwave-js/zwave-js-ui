...
describe('ZwaveClient service wiring: association/group facades, buildVirtualValueId, startup restoration, node removal, socket projection', () => {
	...
	describe('Layer 13 node-registry and socket-adapter facade wiring', () => {
		it('keeps legacy node facade methods behaviorally wired', () => {
			...
			const zwaveNode = {
				...
				getEndpoint: () => ({ getCCVersion: () => 1 }),
				getCCVersion: () => 1,
				getDefinedValueIDs: () => [],
				getValueTimestamp: () => 123,
				hasDeviceConfigChanged: () => false,
			}
			;(zwave as any)._driver = {
				controller: {
					nodes: new Map([[2, zwaveNode]]),
					getPrioritySUCReturnRouteCached: () => undefined,
					getCustomSUCReturnRoutesCached: () => undefined,
				},
			}
			...
			const node = (zwave as any)._createNode(2)
			;(zwave as any)._onNodeStatus(zwaveNode, {
				updateStatusOnly: true,
				updateInterviewStage: true,
			})
			;(zwave as any)._onNodeValueAdded(zwaveNode, valueArgs)

			expect(zwave.nodes.get(2)).toBe(node)
			expect(node).toMatchObject({
				status: 'Alive',
				interviewStage: 'Complete',
			})
			expect(node.values['37-0-event']).toMatchObject({
				id: '2-37-0-event',
				value: true,
			})
		})

		it('applies node settings through the public facade', async () => {
			const zwave = realZwave()
			const zwaveNode = {
				name: '',
				location: '',
			...
			;(zwave as any).driverInfo = { name: '0x1313' }
			;(zwave as any).storeNodes = {}
			zwave.nodes.set(2, node as any)
			zwave.driverReady = true
			const options = {
			...
			await expect(zwave.setNodeName(2, 'Kitchen')).resolves.toBe(true)
			await expect(zwave.setNodeLocation(2, 'First floor')).resolves.toBe(
				true,
			)
			zwave.setNodeDefaultSetValueOptions(2, options)
			...
			expect(zwaveNode).toMatchObject({
				name: 'Kitchen',
			...
			const persisted = JSON.parse(
				readFileSync(
					path.join(getTestStoreDir(), 'nodes.json'),
					'utf8',
				),
			)
			expect(persisted['0x1313'][2]).toEqual({
				name: 'Kitchen',
				loc: 'First floor',
			})
		})

		it('drops an init socket delivery after the registry is replaced', async () => {
			const zwave = realZwave()
			const registry = (zwave as any)._nodeRegistry
			const generation = (zwave as any)._nodeGeneration
			const socket = { emit: vi.fn() }
			let resolveSockets: (sockets: Array<typeof socket>) => void
			const sockets = new Promise<Array<typeof socket>>((resolve) => {
				resolveSockets = resolve
			})
			vi.spyOn((zwave as any).socket, 'fetchSockets').mockReturnValue(
				sockets,
			)

			const pending = (zwave as any).sendInitToSockets(
				registry,
				generation,
			)
			zwave.init()
			resolveSockets([socket])
			await pending

			expect(socket.emit).not.toHaveBeenCalled()
		})

		it('drops a same-driver-generation ready continuation after hard-reset init', async () => {
			const zwave = realZwave()
			...
			;(zwave as any)._driver = { controller }
			;(zwave as any).driverInfo = {}
			vi.spyOn(
				zwave as any,
				'_startScheduledConfigCheck',
			).mockImplementation(() => {})
			vi.spyOn(
				zwave as any,
				'_updateControllerStatus',
			).mockImplementation(() => {})
			vi.spyOn(
				(zwave as any)._inclusionCoordinator,
				'syncFromDriver',
			).mockImplementation(() => {})
			vi.spyOn(
				(zwave as any)._inclusionCoordinator,
				'reinstallUserCallbacks',
			).mockImplementation(() => {})
			...
			vi.spyOn(
				(zwave as any)._nodeRegistry,
				'restorePersistedNodes',
			).mockReturnValue(restore)
			const createBroadcastNodes = vi.spyOn(
				zwave as any,
				'_createBroadcastNodes',
			)

			const ready = (zwave as any)._onDriverReady(
				(zwave as any)._driverLifecycle.generation,
			)
			await vi.waitFor(() => {
				expect(
					(zwave as any)._nodeRegistry.restorePersistedNodes,
				).toHaveBeenCalledOnce()
			})
			zwave.init()
			expect(controller.listenerCount('node found')).toBe(0)
			expect(controller.listenerCount('node added')).toBe(0)
			expect(controller.listenerCount('node removed')).toBe(0)
			expect(controller.listenerCount('statistics updated')).toBe(0)
			resolveRestore()
			await ready

			expect(createBroadcastNodes).not.toHaveBeenCalled()
		})
	})
