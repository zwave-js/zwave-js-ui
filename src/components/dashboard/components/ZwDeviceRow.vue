<template>
	...
	>
		...
		<!-- value -->
		<span
			v-if="hasCol('value') || viewport < 600"
			class="zw-row__cell"
		>
			<ZwCompactPrimary
				:device="device"
		...
		<!-- power -->
		<span
			v-if="hasCol('power') && viewport >= 600"
			class="zw-row__cell"
		>
		...
		<!-- signal -->
		<span
			v-if="hasCol('signal') && viewport >= 600"
			class="zw-row__cell"
		>
			<component
				:is="signalIcon"
...
const rowStyle = computed(() => ({
	gridTemplateColumns: gridTemplateB(props.viewport, props.columns),
}))
