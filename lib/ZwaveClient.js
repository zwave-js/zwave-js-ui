...
function onNodeValueUpdated (zwaveNode, args) {
  ...
  debug(
    `Node ${zwaveNode.id}: value updated: ${getValueID(args)}`, args.prevValue, '=>', args.newValue
  )
...
ZwaveClient.prototype.removeAllAssociations = async function (nodeId) {
  ...
  if (zwaveNode) {
    try {
      const associationsGroups = await this.driver.controller.getAssociations(nodeId)
      ...
      for (const [groupId, associations] of associationsGroups) {
        if (associations.length > 0) {
          ...
          debug(`Assocaitions: Removed ${associations.length} associations from Node ${nodeId} group ${groupId}`)
        }
      }
    } catch (error) {
      debug(
...
ZwaveClient.prototype.removeNodeFromAllAssociations = async function (nodeId) {
  ...
  if (zwaveNode) {
    ...
    debug(`Node ${nodeId} not found when calling 'removeNodeFromAllAssociations'`)
  }
}
/**
