...
function onNodeInterviewCompleted(zwaveNode) {
  ...
  logger.info(`Node ${zwaveNode.id}: interview completed, all values are updated`)
...
function onNodeWakeUp(zwaveNode, oldStatus) {
  logger.info(
    `Node ${zwaveNode.id} is ${oldStatus === NodeStatus.Unknown ? '' : 'now '
    }awake`
  )
...
function onNodeNotification(zwaveNode, notificationLabel, parameters) {
  logger.info(
    `Node ${zwaveNode.id}: notification: ${notificationLabel} ${parameters ? 'with ' + parameters.toString() : ''
    }`
  )
...
function addValue(zwaveNode, zwaveValue) {
  ...
  if (!node) {
    ...
    logger.info(`Node ${zwaveNode.id}: value added ${valueId.id} => ${valueId.value}`)
...
function removeValue(zwaveNode, args) {
  ...
  if (toRemove) {
    ...
    logger.info('ValueRemoved: %s from node %d', idString, zwaveNode.id)
  } else {
...
ZwaveClient.prototype.heal = function () {
  ...
  try {
    ...
    logger.error('Error while doing scheduled network heal', error.message)
  }
...
ZwaveClient.prototype.addAssociations = async function (
  ...
  if (zwaveNode) {
    try {
      ...
      logger.warn(`Error while adding associations to ${nodeId}: ${error.message}`)
    }
  } else {
  ...
  if (!this.driverReady) {
    ...
    try {
      ...
      this.driver.destroy().catch(err => {
        logger.error('Error while destroing driver ' + err.message)
      })
...
ZwaveClient.prototype.writeValue = async function (valueId, value) {
  if (this.driverReady) {
    ...
    try {
      ...
      logger.error(`Error while writing ${value} on ${valueId.id}: ${error.message}`)
    }
