...
const logger = require('./logger.js').setup('zwave')
...
ZwaveClient.prototype.heal = function () {
  ...
  try {
    ...
    logger.error(`Error while doing scheduled network heal ${error.message}`, error)
  }
...
ZwaveClient.prototype.writeValue = async function (valueId, value) {
  if (this.driverReady) {
    ...
    try {
      ...
      logger.error(
        `Error while writing ${value} on ${valueId.id}: ${error.message}`, error
      )
    }
