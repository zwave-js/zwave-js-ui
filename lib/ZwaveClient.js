...
function onNodeStatus (zwaveNode) {
  ...
  if (node) {
    ...
    logger.error(Error(`Received update from node ${zwaveNode.id} that doesn't exists`))
  }
}
...
function onNodeReady (zwaveNode) {
  ...
  if (!node) {
    logger.error(`Node ${zwaveNode.id} ready event called on a node that doesn't exists in memory`)
    return
  }
  ...
  if (!node.inited) {
    ...
    // node is inited when all it's info are parsed and all values added
    node.inited = true
  }

  // don't set the node as ready before it is inited, to prevent discovery
  node.ready = true
...
function addNode (zwaveNode) {
  ...
  if (this.nodes[nodeId] && !this.nodes[nodeId].failed) {
    logger.error('Error while adding node ' + nodeId, Error('node has been added twice'))
    return
  }
 ...
 *
 * @param {any} devices List of devices `"<deviceId>" : <deviceObject>`
 ...
 *
 * @returns {Array<number[]>} The nodes array where `nodeId` is the array index and the value is the array
 ...
 *
 * @param {number} sceneid The scene id
 * @param {any} valueId internal valueId
 ...
 *
 * @param {number} sceneid The scene id
 * @param {any} valueId The valueId to remove
 * @throws Error if args valueid isn't valid
 */
 ...
 *
 * @param {any} valueId Zwave valueId object
