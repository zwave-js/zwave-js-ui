...
const VAR_TYPES = {
  ...
  'bitset': (v, valueId) => {
    ...
    for (const bit in valueId.bitSetIds) {
      valueId.bitSetIds[parseInt(bit)].value = binaryValue.charAt(parseInt(8 - bit)) === '1'
    }
...
function valueAdded (nodeid, comclass, valueId) {
  ...
  if (!ozwnode) {
    ...
    //     'value': 1,
    //     'bitSetIds': {
    //       '1': {label: 'Battery', help: 'Send Battery Report', value: true},
    //       '5': {label: 'Ultraviolet', help: 'Send Ultraviolet Report', value: false}
    //     }
    //   }
    // }
...
ZwaveClient.prototype.callApi = async function (apiName, ...args) {
  ...
  if (this.connected) {
    try {
      ...
      var useCustom = typeof this[apiName] === 'function' && (apiName.toLowerCase().includes('scene') || apiName === 'setNodeName' || apiName === 'setNodeLocation' || apiName === 'refreshNeighborns')
