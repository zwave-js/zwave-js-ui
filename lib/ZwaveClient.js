...
function onNodeReady (zwaveNode) {
  ...
  if (!node.ready) {
    ...
    // rerun addValues with Discovery on.
    // the current flow prevents us from discoverying properly as not
    // all values been prebuild
    for (const zwaveValue of values) {
      discoverDevice.call(this, zwaveNode, zwaveValue)
    }
...
function discoverDevice (zwaveNode, zwaveValue) {
  const node = this.nodes[zwaveNode.id]

  if (!node) {
    debug('discoverDevice: no such node: ' + zwaveNode.id, 'error')
  } else {
    ...
    debug(`Node ${zwaveNode.id}: hass Discovery for ${valueId.id}`)

    this.emit('readyForDiscovery', valueId, node)
  }
}
