...
function valueRemoved(nodeid, comclass, instance, index) {
  ...
  */
  ...
  /**
  * Method used to activate a scene (fix for activateScene not working properly)
  */
  ZwaveClient.prototype.activateScene = function (sceneId) {
    var values = this.client.sceneGetValues(sceneId);

    for (var i = 0; values && i < values.length; i++) {
      this.client.setValue(values[i], values[i].value)
    }
      ...
      if(typeof this.client[apiName] === 'function'){
        try {
          ...
          switch(apiName){
            ...
            // this.scenes.push({sceneid: id, label: args[0], values: []});
            // updateStore = true;
            // result = true;
            // break;
            // case 'removeScene':
            // var index = this.scenes.findIndex(s => s.sceneid == args[0]);
            // if(index >= 0) {
            ...
            // break;
            // case 'getScenes':
            // result = this.scenes.map(s => {sceneid: s.sceneid, label: s.label});
            // break;
            // case 'sceneGetValues':
            // var scene = this.scenes.find(s => s.sceneid == args[0]);
            // if(scene) result = scene.values;
            // else result = [];
            // break;
            ...
            // break;
            case: 'activateScene':
            result = this.activateScene(...args);
            break;
            default:
