...
function onNodeValueUpdated (zwaveNode, args, isNotification) {
  ...
  logger.info(
    `Node ${zwaveNode.id}: value ${isNotification ? 'notification' : 'updated'}: ${getValueID(args)} ${
      args.prevValue
    } => ${args.newValue}`
  )
    ...
    const zwaveOptions = Object.assign(
      {
        ...
        logConfig: { // https://github.com/zwave-js/node-zwave-js/blob/b4e7a02490cfff4f0d4873ede1d318e36b8040b8/docs/api/driver.md#LogConfig
