...
function init (cfg, socket) {
  ...
  if (cfg.networkKey) { options.NetworkKey = cfg.networkKey }

  if (cfg.configPath) { options.ConfigPath = cfg.configPath }

  if (cfg.assumeAwake) { options.AssumeAwake = cfg.assumeAwake }
  ...
  if (!this.cfg.saveConfig) {
    fs.readdir(utils.getPath(true), (err, files) => {
      if (err) debug('Error while reading configuration dir', err.message)
        ...
        files.forEach(file => {
          file = file.split('/').pop()
          if (/zwcfg_[\w]+.xml/g.test(file) || file === 'zwscene.xml') { fs.unlinkSync(file) }
        })
      }
    })
  }
...
function nodeRemoved (nodeid) {
  ...
  if (this.nodes[nodeid]) { this.nodes[nodeid] = null }
...
ZwaveClient.prototype.addSceneValue = function (sceneid, ...args) {
  ...
  if (this.nodes.length < valueId.node_id || !this.nodes[valueId.node_id]) throw Error('Node not found')
    ...
    if (!valueId) throw Error('No value found with given valueId')
      ...
      if (index < 0) { scene.values.push(valueId) }
    }
  }
...
ZwaveClient.prototype.callApi = function (apiName, ...args) {
  ...
  if (this.connected) {
    ...
    if (typeof this.client[apiName] === 'function' || apiName === 'setScenes') {
      try {
        ...
        if (apiName.toLowerCase().includes('scene')) { result = this[apiName](...args) } else { result = this.client[apiName](...args) }
      } catch (e) {
        err = e.message
      }
  ...
  if ((apiName === 'enablePoll' || apiName === 'disablePoll') && args[0] && args[0].node_id) {
    ...
    if (this.nodes[nid] && this.nodes[nid][vid]) { this.nodes[nid][vid].is_polled = this.client.isPolled(args[0]) }
  }
