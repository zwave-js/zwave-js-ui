...
function onNodeReady(zwaveNode) {
  ...
  logger.info(
    `Node ${node.id} ready: ${node.manufacturer} - ${node.productLabel
...
function onNodeValueUpdated(zwaveNode, args) {
  ...
  logger.info(
    `Node ${zwaveNode.id}: value ${args.stateless ? 'notification' : 'updated'
    }: ${getValueID(args)} ${args.stateless ? args.newValue : `${args.prevValue} => ${args.newValue}`
    }`
  )
...
function initNode(zwaveNode) {
  ...
  node.dbLink = `https://devices.zwave-js.io/?jumpTo=${hexIds[0]}:${hexIds[2]}:${hexIds[1]}:${node.firmwareVersion || '0.0'}`
...
ZwaveClient.prototype.writeBroadcast = async function (valueId, value) {
  if (this.driverReady) {
    try {
      ...
      logger.error(
        `Error while sending broadcast ${value} to CC ${valueId.commandClass} ${valueId.property
...
ZwaveClient.prototype.writeMulticast = async function (nodes, valueId, value) {
  if (this.driverReady) {
    ...
    try {
      ...
      logger.error(
        `Error while sending multicast ${value} to CC ${valueId.commandClass} ${valueId.property
        } ${valueId.propertyKey || ''}: ${error.message}`
      )
    }
