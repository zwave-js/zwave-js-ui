...
ZwaveClient.prototype.writeBroadcast = async function (valueId, value) {
  if (this.driverReady) {
    try {
      ...
      debug(`Error while sending broadcast ${value} to CC ${valueId.commandClass} ${valueId.property} ${valueId.propertyKey || ''}`)
    }
  }
}
...
ZwaveClient.prototype.writeMulticast = async function (nodes, valueId, value) {
  if (this.driverReady) {
    try {
      ...
      debug(`Error while sending multicast ${value} to CC ${valueId.commandClass} ${valueId.property} ${valueId.propertyKey || ''}`)
    }
  }
}
