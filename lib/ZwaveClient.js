...
function driverReady (homeid) {
  ...
  var zwcfg_path = utils.joinPath(storeDir, 'zwcfg_' + homeHex + '.xml')
  ...
  if (this.zwcfg_nodes.length === 0) {
    fs.readFile(zwcfg_path, 'utf8', function (err, zwcfg) {
      if (!err) {
        ...
        if (this.zwcfg_nodes.length > 0) {
          jsonStore.put(store.nodes, this.zwcfg_nodes).catch(debug)
        }
      }
    }.bind(this))
  }
...
function nodeReady (nodeid, nodeinfo) {
  ...
  if (ozwnode) {
    ...
    for (const id in ozwnode.values) {
      this.emit('valueChanged', ozwnode.values[id], ozwnode, id)
    }
