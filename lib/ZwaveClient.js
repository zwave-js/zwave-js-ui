...
ZwaveClient.prototype.setPollInterval = function (valueId, interval) {
  if (this.driver && !this.closed) {
    ...
    this.pollIntervals[vID] = setTimeout(tryPoll.bind(this, valueId, interval), interval * 1000)
  } else {
    throw Error('Driver is closed')
  }
}
...
async function tryPoll (valueId, interval) {
  try {
    ...
    logger.error(`Error while polling value ${getValueID(valueId, true)}: ${error.message}`)
  }
