...
const { Driver, NodeStatus } = require('zwave-js')
...
function init (cfg, socket) {
  ...
  this.driver = new Driver(cfg.port, {
    cacheDir: storeDir,
...
function driverError (error) {
  this.error = 'Driver error ' + error.message
  this.status = ZWAVE_STATUS.driverFailed
  debug(this.error, this.ozwConfig)
}
...
function onInclusionStarted (secure) {
  debug(`${secure ? 'Secure' : 'Non-secure'} inclusion started`)
}

function onExclusionStarted () {
  debug('Exclusion started')
}

function onInclusionStopped () {
  debug('Inclusion stopped')
}

function onExclusionStopped () {
  debug('Exclusion stopped')
}

function onInclusionFailed () {
  debug('Inclusion failed')
}

function onExclusionFailed () {
  debug('Exclusion failed')
}
...
function onHealNetworkProgress (progress) {
  ...
  if (healedNodes.length === toHeal.length) {
    debug(`Healing process COMPLETED. Healed ${toHeal.length} nodes`)
  } else {
    debug(`Healing process IN PROGRESS. Healed ${healedNodes.length} nodes`)
  }
}

function onHealNetworkDone (result) {
  debug(`Healing process COMPLETED. Healed ${result.length} nodes`)
}
...
function onNodeReady (zwaveNode) {
  ...
  node.ready = true
  onNodeStatus.call(this, zwaveNode.id, NODE_STATUS.alive)
...
function onNodeWakeUp (zwaveNode, oldStatus) {
  ...
  onNodeStatus.call(this, zwaveNode.id, NODE_STATUS.awake)
}
...
function onNodeSleep (
  ...
  onNodeStatus.call(this, zwaveNode.id, NODE_STATUS.asleep)
}
...
function onNodeDead (
  ...
  onNodeStatus.call(this, zwaveNode.id, NODE_STATUS.dead)
  debug(
    `Node ${zwaveNode.id} is ${
      oldStatus === NodeStatus.Unknown ? '' : 'now '
    }dead`
...
function updateValueMetadata (zwaveNode, zwaveValue, zwaveValueMeta) {
  var valueId = {
    ...
    writable: zwaveValueMeta.writable,
  ...
  if (zwaveValueMeta.type === 'number') {
    ...
    valueId.default = zwaveValueMeta.default
...
function updateValue (zwaveNode, args) {
  ...
  if (!node) {
    ...
    var valueId = node[getValueID(args)]

    if (valueId) {
      valueId.value = args.newValue

      this.emit('valueChanged', args, node, args.prevValue !== args.newValue)
    }
    node.lastActive = Date.now()
  }
}
 ...
 *
 * @param {Integer} sceneid The scene id
 * @param {Array} args Array or argument. Can be `[{valueid}, value, ?timeout]` or `[nodeId, class_id, instance, index, value, ?timeout]`
...
ZwaveClient.prototype.writeValue = function (valueId, value) {
  if (this.driverReady) {
    try {
      this.getNode(valueId.nodeId).setValue(valueId, value)
    } catch (error) {
      debug(
