...
ZwaveClient.prototype.writeBroadcast = async function (valueId, value) {
  if (this.driverReady) {
    try {
      ...
      logger.error(`Error while sending broadcast ${value} to CC ${valueId.commandClass} ${valueId.property} ${valueId.propertyKey || ''}: ${error.message}`)
    }
  }
}
...
ZwaveClient.prototype.writeMulticast = async function (nodes, valueId, value) {
  if (this.driverReady) {
    try {
      ...
      logger.error(`Error while sending multicast ${value} to CC ${valueId.commandClass} ${valueId.property} ${valueId.propertyKey || ''}: ${error.message}`)
    }
  }
}
