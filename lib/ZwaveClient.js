...
function onNodeInterviewFailed (zwaveNode, args) {
  logger.error(
    `Interview attemp ${args.attempt} of node ${zwaveNode.id} has failed: ${args.errorMessage}`
  )
...
ZwaveClient.prototype.refreshValues = function (nodeId) {
  try {
    this.getNode(nodeId).refreshValues()
  } catch (error) {
    logger.error(
      `Error while refreshing values: ${error.message}`,
      error
    )
