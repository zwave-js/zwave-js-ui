...
const { ZWaveNotificationCallbackArgs_NotificationCC, ZWaveNotificationCallbackArgs_EntryControlCC } = require('@zwave-js/CommandClass')
...
function onNodeNotification (zwaveNode, ccId, args) {
  ...
  if (args instanceof ZWaveNotificationCallbackArgs_NotificationCC) {
    valueId.property = args.label
    valueId.propertyKey = args.eventLabel

    data = parseNotification(args.parameters)
  } else if (args instanceof ZWaveNotificationCallbackArgs_EntryControlCC) {
    valueId.property = args.eventType
    valueId.propertyKey = args.dataType
    data = args.eventData instanceof Buffer ? utils.buffer2hex(args.eventData) : args.eventData
  }

  logger.log('info',
    'Node %d CC %s %o', zwaveNode.id, valueId.commandClassName, args
  )
