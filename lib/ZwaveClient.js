...
function onNodeNotification (zwaveNode, notificationLabel, parameters) {
  debug(`Node ${zwaveNode.id}: notification: ${notificationLabel} ${parameters ? 'with ' + parameters.toString() : ''}`)
...
function removeNode (nodeid) {
  ...
  if (node) {
    ...
    this.addEmptyNodes()
    this.emitEvent(socketEvents.nodeRemoved, node)
  }
}
...
function addValue (zwaveNode, zwaveValue) {
  ...
  if (!node) {
    ...
    var valueId = updateValueMetadata.call(
      this,
      zwaveNode,
      zwaveValue,
      zwaveValueMeta
...
function getValueID (v, nodeId) {
  return `${nodeId >= 0 ? nodeId + '-' : ''}${v.commandClass}-${v.endpoint ||
...
ZwaveClient.prototype._activateScene = function (sceneId) {
  ...
  for (var i = 0; values && i < values.length; i++) {
    var fun = wrapFunction(this.writeValue, this, [values[i], values[i].value])
