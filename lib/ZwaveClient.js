...
function ZwaveClient (config, socket) {
  ...
  if (Array.isArray(this.storeNodes)) {
    ...
    for (const n of this.storeNodes) {
      if (n) {
        this.storeNodes[n.id] = n
      }
    }
...
function onNodeReady (zwaveNode) {
  ...
  this.emit('event', eventEmitter.node, 'node ready', this.nodes.get(zwaveNode.id))
...
function onNodeWakeUp (zwaveNode, oldStatus) {
  ...
  onNodeStatus.call(this, zwaveNode)
  this.emit('event', eventEmitter.node, 'node wakeup', this.nodes.get(zwaveNode.id))
}
...
function onNodeSleep (zwaveNode, oldStatus) {
  ...
  onNodeStatus.call(this, zwaveNode)
  this.emit('event', eventEmitter.node, 'node sleep', this.nodes.get(zwaveNode.id))
}
...
function onNodeAlive (zwaveNode, oldStatus) {
  ...
  this.emit('event', eventEmitter.node, 'node alive', this.nodes.get(zwaveNode.id))
}
...
function onNodeDead (zwaveNode, oldStatus) {
  ...
  this.emit('event', eventEmitter.node, 'node dead', this.nodes.get(zwaveNode.id))
}
...
ZwaveClient.prototype._setNodeName = async function (nodeid, name) {
  ...
  if (this.nodes.has(nodeid)) this.nodes.get(nodeid).name = name
  ...
  this.emit('nodeStatus', this.nodes.get(nodeid))
...
ZwaveClient.prototype._setNodeLocation = async function (nodeid, loc) {
  ...
  if (this.nodes.has(nodeid)) this.nodes.get(nodeid).loc = loc
  else throw Error('Invalid Node ID')
