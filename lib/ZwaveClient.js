...
function onNodeReady (zwaveNode) {
  ...
  for (const zwaveValue of values) {
    addValue.call(this, zwaveNode, zwaveValue, wasReady)
  }
 ...
 * @param { import('zwave-js').ZWaveNode } zwaveNode
 * @param { import('zwave-js').TranslatedValueID } zwaveValue
 * @param {boolean} ignoreUpdates don't log updates or emit events
 */
function addValue (zwaveNode, zwaveValue, ignoreUpdates) {
  const node = this.nodes[zwaveNode.id]
