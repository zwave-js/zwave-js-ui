...
function onNodeAdded (zwaveNode) {
  ...
  this.emit('event', eventEmitter.controller, 'node added', this.nodes[zwaveNode.id])
}
...
function onNodeRemoved (zwaveNode) {
  ...
  this.emit('event', eventEmitter.controller, 'node removed', this.nodes[zwaveNode.id])
...
function onNodeInterviewCompleted (zwaveNode) {
  ...
  this.emit('event', eventEmitter.node, 'node interview completed', this.nodes[zwaveNode.id])
}
...
function onNodeValueAdded (
  ...
  this.emit('event', eventEmitter.node, 'node value added', this.nodes[zwaveNode.id], args)
}
...
function onNodeValueRemoved (zwaveNode, args) {
  ...
  this.emit('event', eventEmitter.node, 'node value removed', this.nodes[zwaveNode.id], args)
}
...
function onNodeMetadataUpdated (zwaveNode, args) {
  ...
  this.emit('event', eventEmitter.node, 'node metadata updated', this.nodes[zwaveNode.id], args)
}
...
function onNodeNotification (zwaveNode, notificationLabel, parameters) {
  ...
  this.emit('event', eventEmitter.node, 'node notification', this.nodes[zwaveNode.id], notificationLabel, parameters)
}
...
function onNodeFirmwareUpdateProgress (
  ...
  this.emit('event', eventEmitter.node, 'node firmware update progress', this.nodes[zwaveNode.id], sentFragments, totalFragments)
}
...
function onNodeFirmwareUpdateFinished (zwaveNode, status, waitTime) {
  ...
  this.emit('event', eventEmitter.node, 'node firmware update finished', this.nodes[zwaveNode.id], status, waitTime)
}
...
ZwaveClient.prototype.removeAssociations = async function (
  ...
  if (zwaveNode) {
    try {
      debug(
        `Assocaitions: Removing associations from Node ${nodeId} Group ${groupId}: ${associations}`
      )
