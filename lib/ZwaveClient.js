...
function onNodeFirmwareUpdateProgress (
  ...
  updateControllerStatus.call(this, `Node ${zwaveNode.id} firmware update IN PROGRESS: ${sentFragments}/${totalFragments}`)
}
...
function onNodeFirmwareUpdateFinished (
  ...
  updateControllerStatus.call(this, `Node ${zwaveNode.id} firmware update FINISHED: Status ${status}, Time: ${waitTime}`)
}
...
function updateValueMetadata (zwaveNode, zwaveValue, zwaveValueMeta) {
  var valueId = {
    ...
    genre: zwaveValue.commandClass === 112 ? 'config' : (zwaveValue.commandClass < 94 ? 'user' : 'system'),
...
ZwaveClient.prototype._activateScene = function (sceneId) {
  ...
  for (var i = 0; values && i < values.length; i++) {
    var fun = wrapFunction(this.writeValue, this, [
      values[i],
      values[i].value
    ])
...
ZwaveClient.prototype.isFailedNode = async function (nodeId) {
  if (this.driver && !this.closed) {
    var result = await this.driver.controller.isFailedNode(nodeId)
    this.nodes[nodeId].failed = result
    var zwaveNode = this.getNode(nodeId)
    onNodeStatus.call(this, zwaveNode)
    return result
  }
...
ZwaveClient.prototype.refreshInfo = async function (nodeId) {
  if (this.driver && !this.closed) {
    ...
    if (!zwaveNode) {
      throw Error('No Zwave node found with given id')
    }
...
ZwaveClient.prototype.beginFirmwareUpdate = async function (nodeId, data, target = 0) {
  if (this.driver && !this.closed) {
    var zwaveNode = this.getNode(nodeId)
