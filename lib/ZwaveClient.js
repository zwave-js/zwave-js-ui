...
function valueAdded(nodeid, comclass, valueId) {
  ...
  debug("ValueAdded", valueId.value_id);
...
function valueChanged(nodeid, comclass, valueId) {
  ...
  if (!ozwnode) {
    ...
    if (ozwnode.ready) {
      ...
      debug(`zwave node ${nodeid}: changed: ${comclass}:${valueId.label}:${oldst} -> ${valueId.value}`);
...
function valueRemoved(nodeid, comclass, instance, index) {
  ...
  function nodeReady(nodeid, nodeinfo) {
    ...
    if (ozwnode) {
      ...
      debug('node ready', nodeid, nodeinfo);
    }
  }
  ...
  function scanComplete() {
    ...
    debug('Network scan complete. Found:', this.nodes.length, 'nodes');
  }
