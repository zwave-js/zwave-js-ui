...
const CREDENTIALS_KEY_FILE = joinPath(appConfig.storeDir, appConfig.credentialsKey)
...
CredentialsStore.init = async () => {
  ...
  if (!credentials[keys.USERS] || credentials[keys.USERS].length === 0) {
    ...
    credentials[keys.USERS] = [{ username: appConfig.defaultUser, password: appConfig.defaultPsw, _id: uniqid() }]
    await update()
  }
}
