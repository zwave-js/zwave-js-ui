...
function onNodeStatus (node) {
  if (node.ready && this.config.hassDiscovery) {
    ...
    for (const id of getSanitizedPriorityCC(node.values)) {
...
function getSanitizedPriorityCC (values) {
  ...
  for (const id in values) {
    if (!ignoreDiscovery[values[id].commandClassName].includes(values[id].Property)) {
