...
function setBinarySensorPayload (devClass, reversePayload = false) {
  ...
  cfg.discovery_payload.device_class =
    Constants.deviceClass.sensor_binary[devClass]
  if (reversePayload) {
    ...
    switch (cmdClass) {
      ...
      case CommandClasses['Binary Sensor']:
        ...
        switch (sensorTypeName) {
          ...
          case 'contact':
          case 'water':
            cfg = setBinarySensorPayload('MOISTURE')
            break
          // safety - normal
          case 'co':
          case 'co2':
          case 'tamper':
            cfg = setBinarySensorPayload('SAFETY')
            break
          // problem - normal
          case 'alarm':
            cfg = setBinarySensorPayload('PROBLEM')
            break
          // connectivity - normal
          case 'router':
            cfg = setBinarySensorPayload('CONNECTIVITY')
            break
          // battery - normal
          case 'battery_low':
            cfg = setBinarySensorPayload('BATTERY')
            break
          default:
      ...
      case CommandClasses.Notification:
        ...
        if (valueId.list && valueId.states.length === 2) {
          ...
          switch (valueId.propertyKeyName) {
            case 'Access Control':
              cfg = setBinarySensorPayload('LOCK')
              off = 23 // Closed state
              break
            case 'Cover status':
              cfg = setBinarySensorPayload('OPENING')
              break
            case 'Door state':
              cfg = setBinarySensorPayload('DOOR')
            ...
            case 'Heat sensor status':
              cfg = setBinarySensorPayload('HEAT')
              break
            case 'Motion sensor status':
              cfg = setBinarySensorPayload('MOTION')
              break
            case 'Alarm status':
              cfg = setBinarySensorPayload('PROBLEM')
              break
            ...
            case 'Sensor status':
              switch (valueId.propertyName) {
                case 'Smoke Alarm':
                  cfg = setBinarySensorPayload('SMOKE')
                  break
                case 'Water Alarm':
                  cfg = setBinarySensorPayload('MOISTURE')
                  break
