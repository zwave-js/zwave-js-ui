...
function getMappedValuesTemplate (modeMap, defaultValue) {
  ...
  for (const key in modeMap) map.push(`${typeof modeMap[key] === 'number' ? modeMap[key] : '"' + modeMap[key] + '"'}: "${key}"`)
...
Gateway.prototype.parsePayload = function (payload, valueId, valueConf) {
  try {
    payload = typeof payload === 'object' && payload.hasOwnProperty('value') ? payload.value : payload
    ...
    if (hassDevice) {
      ...
      if (valueId.list) {
        ...
        } else if (valueId.commandClass === CommandClasses['Thermostat Mode'] && hassDevice.mode_map) {
