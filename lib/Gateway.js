...
function addRgbColorSwitch (node, valueId) {
  ...
  */
  ...
  const whiteValue = node.values[`51-${valueId.endpoint}-currentcolor-0`]

  const currentColor = `51-${valueId.endpoint}-currentcolor`
  const currentColorValue = node.values[currentColor]

  // if whitevalue exists, use currentColor value to get/set white
  if (whiteValue && currentColorValue) {
    cfg.values.push(currentColorValue)

    cfg.discovery_payload.color_temp_state_topic = this.mqtt.getTopic(this.valueTopic(node, currentColorValue))
    cfg.discovery_payload.color_temp_command_topic = cfg.discovery_payload.color_temp_state_topic + '/set'
