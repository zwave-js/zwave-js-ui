...
function parsePayload (payload, value, valueConf, config) {
  ...
  if (value.type === 'list' && value.values.indexOf(payload) < 0 && this.discovered[value.value_id]) {
  ...
  if (hassID && !node.hassDevices[hassID]) { // discover the device
    ...
    if (payload) {
      ...
      for (let k = 0; k < hassDevice.values.length; k++) {
        this.discovered[k] = hassDevice
      }

      node.hassDevices[hassID] = hassDevice

      const discoveryTopic = `${hassDevice.type}/${nodeName}/${hassDevice.object_id}/config`
