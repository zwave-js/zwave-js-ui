 ...
 * @param {import('../types').Z2MValueId} valueId the valueId of the `hexColor` Color CC
 * @returns {any} a configuration if the rgb dimmer is added
 */
function addRgbColorSwitch (node, valueId) {
  ...
  } else if (valueId.endpoint === 0 && node.values['38-1-currentValue']) {
  ...
  */
  ...
  if (brightnessValue || switchValue) {
    discoveredStateTopic = this.mqtt.getTopic(
      this.valueTopic(node, node.values[brightnessValue || switchValue])
    )
    discoveredCommandTopic =
      this.mqtt.getTopic(
        this.valueTopic(node, node.values[brightnessValue || switchValue], true).targetTopic
      ) + '/set'
  }

  if (brightnessValue) {
    cfg.values.push(brightnessValue)
  ...
  const whiteId = `51-${valueId.endpoint}-currentcolor-0`
  const whiteValue = node.values[whiteId]

  // Use color_value_* from HASS discovery as white switch
  if (whiteValue) {
    cfg.values.push(whiteId)
    cfg.values.push(whiteValue.targetValue)

    const topics = this.valueTopic(node, whiteValue, true)

    cfg.discovery_payload.color_temp_state_topic = this.mqtt.getTopic(topics.topic)
    cfg.discovery_payload.color_temp_command_topic = this.mqtt.getTopic(topics.targetTopic) + '/set'

    cfg.discovery_payload.color_temp_command_template =
      "{{ {'warmWhite': ((value - 245)|round(0)), 'coldWhite': (255 - (value - 245))|round(0)), 'red': 0, 'green': 0, 'blue': 0}|to_json }}"
