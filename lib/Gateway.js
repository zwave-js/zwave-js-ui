 ...
 */
function setHassFriendlyName (value, cfg, nodeName, valueId) {
  const labelId = valueId ? valueId.label : cfg.object_id
  switch (value) {
    case 0:
    ...
    case 3:
      // Use PropertyKey, if not exists fall back to propertyName
      return `${nodeName} ${labelId}`
    case 4:
      // Use PropertyKey, if not exists fall back to propertyName
      return `${nodeName.replace('-', ' ')} ${labelId}`
    default:
