...
Gateway.prototype.start = async function () {
  ...
  if (this.zwave) {
    this.zwave.on('readyForDiscovery', onReadyForDiscovery.bind(this))
...
/**
 * Run Descovery after node values added
 */
function onReadyForDiscovery (valueId, node) {
    ...
    switch (cmdClass) {
      ...
      case CommandClasses['Binary Toggle Switch']:
        if (valueId.isCurrentValue) {
          // if (valueId.isCurrentValue && setTopic && getTopic) {
