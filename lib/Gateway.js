 ...
 */
function setHassFriendlyName (value, cfg, node, nodeName) {
  switch (value) {
    case 1:
      return `${node.loc} ${node.name} ${cfg.object_id.replace(/_/g, ' ')}`
    case 2:
      return `${node.name} ${node.loc} ${cfg.object_id.replace(/_/g, ' ')}`
    case 3:
      return `${cfg.object_id.replace(/_/g, ' ')}`
    default:
