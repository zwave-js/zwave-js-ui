...
function onValueChanged (valueId, node, changed) {
  ...
  if (this.zwave) this.zwave.emitEvent(this.zwave.socketEvents.valueUpdated, valueId)
  ...
  if (changed && valueId.list && this.discovered[valueId.id]) {
    var hassDevice = this.discovered[valueId.id]
  ...
  tmpVal =
    valueId.list && this.config.integerList
...
function onNodeStatus (node) {
  ...
  if (this.zwave) this.zwave.emitEvent(this.zwave.socketEvents.nodeUpdated, node)
...
function getValueID (v) {
  return v.id.substr(v.id.indexOf('-') + 1)
}
...
function getMappedValuesTemplate (valueId, modeMap, integerList, defaultValue) {
  ...
  if (integerList) {
    ...
    for (let i = 0; i < valueId.states.length; i++) {
      mapi[i.toString()] = map[valueId.values[i].value]
    }
...
Gateway.prototype.parsePayload = function (payload, valueId, valueConf) {
  try {
    ...
    if (this.discovered[valueId.id]) {
      ...
      else if (valueId.commandClass === 0x29) payload = valueId.value > 0 ? 0 : 0xff
    ...
    //   payload >= 0 &&
    // ) {
    //   payload = valueId.states[payload]
    // }
...
Gateway.prototype.valueTopic = function (node, valueId, returnObject) {
  ...
  var values = this.config.values.filter(v => v.device === node.deviceId)
  ...
  if (topic.length === 0) {
    switch (this.config.type) {
      case 1: // named
        topic.push(node.name ? node.name : NODE_PREFIX + valueId.nodeId)
        topic.push(valueId.commandClassName)
...
Gateway.prototype.discoverValue = function (node, valueId) {
  ...
  try {
    ...
    var type = Constants.commandClass(valueId.commandClass)
    var cfg, payload
    ...
    // Set a unique id for the component
    payload.unique_id =
      'zwavejs2mqtt_' + this.zwave.homeHex + '_' + valueId.id
