...
function setLowerNoSpaces (value) {
  ...
  try {
    if (hassID && !node.hassDevices[hassID]) {
      ...
      if (payload) {
        ...
        // make Object_id compatible with Hass
        hassDevice.object_id = setLowerNoSpaces(hassDevice.object_id)
...
Gateway.prototype.discoverValue = function (node, vId) {
  ...
  try {
    ...
    // make Object_id compatible with Hass
    cfg.object_id = setLowerNoSpaces(cfg.object_id)
