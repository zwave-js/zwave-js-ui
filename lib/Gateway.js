...
Gateway.prototype.valueTopic = function (node, valueId, returnObject) {
  ...
  if (topic.length > 0) {
    ...
    return returnObject
      ? { topic: topic.join('/'), valueConf: valueConf, targetTopic: targetTopic }
      : topic.join('/')
  } else {
    return null
  }
  ...
  try {
    ...
    switch (type) {
      ...
      case 'switch_toggle_multilevel':
        if (valueId.isCurrentValue) {
          ...
          if (
            ...
            cfg.discovery_payload.command_topic = this.mqtt.getTopic(result.targetTopic) + '/set'
            cfg.discovery_payload.position_topic = this.mqtt.getTopic(topic)
            cfg.discovery_payload.set_position_topic = cfg.discovery_payload.command_topic
      ...
      case 'sensor_alarm':
        ...
        cfg = copy(hassCfg.binary_sensor_alarm)
        cfg.object_id += alarmMap[valueId.property]
          ? '_' + alarmMap[valueId.property]
          : ''
        break

      case 'alarm':
        cfg = copy(hassCfg.sensor_generic)
        // FIXME: Constants alarm type
        cfg.object_id = 'alarm_' + Constants.alarmType(valueId.property)
      ...
      case 'battery':
        ...
        if (type === 'sensor_multilevel') {
          ...
          // FIXME: Constants production type
    ...
    if (payload.command_topic === true && result.targetTopic) {
      payload.command_topic = this.mqtt.getTopic(result.targetTopic) + '/set'
    }
