...
Gateway.prototype.parsePayload = function (payload, valueId, valueConf) {
  try {
    ...
    if (this.discovered[valueId.id]) {
      ...
      if (valueId.commandClass === CommandClasses['Binary Toggle Switch']) { payload = 1 } else if (
...
Gateway.prototype.publishDiscovery = function (
  ...
  try {
    ...
    // don't discovery this device when skip is true
    if (!hassDevice.skipDiscovery) {
      if (this.config.payloadType === 2) {
 ...
 */
Gateway.prototype.setDiscovery = function (nodeId, hassDevice, deleteDevice = false) {
  ...
  try {
    if (hassID && !node.hassDevices[hassID]) {
      ...
      if (payload) {
        ...
        hassDevice.skipDiscovery = !!hassDevice.skipDiscovery
