...
function addRgbColorSwitch (node, valueId) {
  ...
  */
  ...
  if (whiteValue && currentColorValue) {
    cfg.values.push(currentColor)
    cfg.values.push(currentColorValue.targetValue)

    const topics = this.valueTopic(node, currentColorValue, true)

    cfg.discovery_payload.color_temp_state_topic = this.mqtt.getTopic(
      topics.topic
    )
    cfg.discovery_payload.color_temp_command_topic = this.mqtt.getTopic(
      topics.targetTopic,
      true
    )
