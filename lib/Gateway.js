...
function addRgbColorSwitch (cfg, node, valueId) {
  ...
  */
  ...
  if (brightnessValue || switchValue) {
    cfg.discovery_payload.state_topic =
      cfg.discovery_payload.brightness_state_topic
    cfg.discovery_payload.command_topic =
      cfg.discovery_payload.brightness_command_topic
  }

  /* HASS do not support Warm White and Cold White, these are exposed as Dimmers
     Add calculated whites, based on RGB scale using a kelvin to RGB translator
  */
  cfg.discovery_payload.white_value_state_topic =
    cfg.discovery_payload.rgb_state_topic

  // mimic the defined kelvin scales
  cfg.discovery_payload.white_value_scale = Constants.deviceClass.rgbWhiteTemp.length()
  cfg.discovery_payload.white_value_template = `{{ ${JSON.stringify(
    Constants.deviceClass.rgbWhiteTemp
  )}[value_json.value] | default(0) }}`
  cfg.discovery_payload.white_value_command_topic =
    cfg.discovery_payload.rgb_command_topic
