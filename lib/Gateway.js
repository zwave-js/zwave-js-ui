...
function addRgbColorSwitch (node, valueId) {
  ...
  if (node.values[`38-${valueId.endpoint}-currentValue-0`]) {
    brightnessValue = `38-${valueId.endpoint}-currentValue-0`
    // Next if is about Fibaro like RGBW which use the endpoint 1 as multilevel
  } else if (valueId.endpoint === 0 && node.values['38-1-currentValue-0']) {
    brightnessValue = '38-1-currentValue-0'
  ...
  */
  ...
  if (node.values[`51-${valueId.endpoint}-currentcolor-0`]) {
    cfg.discovery_payload.color_temp_state_topic = `51-${valueId.endpoint}-currentColor`
    ...
    cfg.discovery_payload.color_temp_command_topic = `51-${valueId.endpoint}-targetColor-0`
