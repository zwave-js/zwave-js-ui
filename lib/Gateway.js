...
async function onBroadRequest (payload) {
  const value = payload.value
  const valueId = payload

  if (payload.value === undefined) {
    debug('No value found in broadcast request')
    return
  }
  ...
  if (typeof error === 'string') {
    debug('Invalid valueId: ' + error)
    return
  }

  await this.zwave.writeBroadcast(valueId, value)
}
...
async function onMulticastRequest (payload) {
  ...
  if (!nodes || nodes.length === 0) {
    debug('No nodes found in multicast request')
    return
  }
  ...
  if (payload.value === undefined) {
    debug('No value found in multicast request')
    return
  }
