...
function getPriorityCCFirst (values) {
  const priorityList = Object.keys(values)
    .filter(
      k =>
        Constants.priorityCommandClass
          .includes(values[k].commandClass
          ))
  let remainingList = []
  /* for (const k in values) {
    if (Constants.priorityCommandClass.includes(values[k].commandClass)) {
      priorityList += k
    } else {
      remainingList += k
    }
...
Gateway.prototype.rediscoverNode = function (nodeID) {
  ...
  if (node) {
    ...
    const prioritizedValues = getPriorityCCFirst(node.values)
    // discover node values (that are not part of a device)
    for (const valueObject in prioritizedValues) {
      for (const id in prioritizedValues[valueObject]) {
        this.discoverValue(node, id)
      }
    }
    this.zwave.sendToSocket(socketEvents.nodeUpdated, node)
