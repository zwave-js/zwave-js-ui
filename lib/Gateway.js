    ...
    switch (cmdClass) {
      ...
      case CommandClasses['Binary Sensor']:
        ...
        switch (sensorTypeName) {
          ...
          case 'smoke':
          case 'gas':
            cfg.discovery_payload.device_class = sensorTypeName
            break
          // reverse
          case 'lock':
      ...
      case CommandClasses.Notification:
        ...
        if (valueId.list && valueId.states.length === 2) {
          ...
          cfg = copy(hassCfg.binary_sensor) // Setup the configuration using default values
          cfg.object_id = valueId.propertyKey
          switch (valueId.propertyKeyName) {
            case 'Access Control':
              cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.LOCK
              off = 23 // Closed state
              break
            case 'Cover status':
              cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.OPENING
              break
            case 'Door state':
              cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.DOOR
            ...
            case 'Hardware status':
              cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.PROBLEM
              break
            case 'Heat sensor status':
              cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.HEAT
              break
            case 'Motion sensor status':
              cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.MOTION
              break
            case 'Alarm status':
              cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.CONNECTIVITY
              break
            ...
            case 'Sensor status':
              switch (valueId.propertyName) {
                case 'Smoke Alarm':
                  cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.SMOKE
                  break
                case 'Water Alarm':
                  cfg.discovery_payload.device_class = Constants.deviceClass.sensor_binary.MOISTURE
                  break
                default:
              }
