 ...
 *
 * @param {object} cfg Hass discovery Configuration
 * @param {object} valueId The device ValueID
 * @param {number} offStateValue The value number to consider off state
*/
function getBinaryPayloadFromSensor (cfg, valueId, offStateValue = 0) {
  const stateKeys = Array.from(
    getMapFromArrayOfStates(valueId.states).keys()
  )
