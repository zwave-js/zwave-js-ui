...
Gateway.prototype.discoverValue = function (node, valueId) {
  ...
  try {
    ...
    const setTopic = result.targetTopic ? this.mqtt.getTopic(result.targetTopic, true) : null
    ...
    switch (cmdClass) {
      ...
      case CommandClasses['Binary Sensor']:
        ...
        if (sensorTypeName) {
          sensorTypeName = this.mqtt.cleanName(sensorTypeName.toLocaleLowerCase())
        }
      ...
      case CommandClasses['Alarm Sensor']:
        ...
        if (valueId.property === 'state') {
          ...
          cfg.object_id += AlarmSensorType[valueId.property]
            ? '_' + AlarmSensorType[valueId.property]
            : ''
        } else {
          return
        }
      ...
      case CommandClasses.Battery:
        ...
        if (cmdClass === CommandClasses['Multilevel Sensor']) {
          ...
          sensor = SensorType[valueId.property]
        } else if (cmdClass === CommandClasses.Meter) { // https://github.com/zwave-js/node-zwave-js/blob/master/packages/zwave-js/src/lib/commandclass/MeterCC.ts
