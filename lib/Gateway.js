 ...
 *
 * @param {node.valueId} values valueId keys
 * @returns An nested array, first value is priority CC values and second the remaining
 */

function getPriorityCCFirst (values) {
  const prioritizedValueIds = []
  for (const id in values) {
    if (Constants.priorityCommandClass.includes(values[id].commandClass)) {
...
function addRgbDimmer (cfg, node, valueId) {
  ...
  if (rgb) {
    ...
    if (whiteValue) {
      const whiteIDWrite = `51-${valueId.endpoint || 0}-hexColor`
      ...
      // mimic 25 white kelvin scales
      cfg.discovery_payload.white_value_scale = 25
      cfg.discovery_payload.white_value_template = `{{ ${JSON.stringify(Constants.deviceClass.rgbWhiteTemp)}[value_json.value] | default(0) }}`
...
function addRgbColorSwitch (cfg, node, valueId) {
  cfg = copy(cfg)
  cfg.discovery_payload.rgb_state_topic = this.mqtt.getTopic(
    this.valueTopic(node, valueId)
  )
  cfg.discovery_payload.rgb_command_topic =
    cfg.discovery_payload.rgb_state_topic + '/set'
  // check if multilevel switch exists
  let brightnessValue = false
  let switchValue = false
  if (node.values.hasOwnProperty(`38-${valueId.endpoint}-currentValue`)) {
    brightnessValue = `38-${valueId.endpoint}-currentValue`
  } else if (node.values.hasOwnProperty(`38-${valueId.endpoint + 1}-currentValue`)) {
    brightnessValue = `38-${valueId.endpoint + 1}-currentValue`
  } else if (node.values.hasOwnProperty(`37-${valueId.endpoint}-currentValue`)) {
    switchValue = `37-${valueId.endpoint}-currentValue`
  }

  // eslint-disable-next-line no-undef
  if (brightnessValue) {
    cfg.discovery_payload.brightness_state_topic = this.mqtt.getTopic(
      this.valueTopic(node, node.values[brightnessValue])
    )
    cfg.discovery_payload.brightness_command_topic = this.mqtt.getTopic(
      this.valueTopic(node, node.values[brightnessValue], true).
        targetTopic
    )
    cfg.discovery_payload.state_topic = false
  } else if (switchValue) {
    cfg.discovery_payload.state_topic = this.mqtt.getTopic(
      this.valueTopic(node, node.values[brightnessValue])
    )
    cfg.discovery_payload.command_topic = this.mqtt.getTopic(
      this.valueTopic(node, node.values[brightnessValue], true).
        targetTopic
    )
    cfg.discovery_payload.state_value_template = '{{ state_value_template }}'
...
Gateway.prototype.rediscoverNode = function (nodeID) {
  ...
  if (node) {
    ...
    // discover node values (that are not part of a device)
    // iterate prioritized first, then the remaining
    for (const groupedValues of getPriorityCCFirst(node.values)) {
      for (const id of groupedValues) {
        this.discoverValue(node, id)
      }
    }
    ...
    switch (cmdClass) {
      ...
      case CommandClasses['Color Switch']:
        if (valueId.property === 'hexColor') {
          cfg = addRgbColorSwitch.call(this, hassCfg.light_rgb_dimmer, node, valueId)

        } else if (valueId.property === 'currentColor') {
          if (valueId.propertyKey <= 1) {
            cfg = copy(hassCfg.light_dimmer)
            cfg.discovery_payload.brightness_scale = valueId.max
          } else return
        } else return
        break
