...
Gateway.prototype.parsePayload = function (payload, value, valueConf) {
  ...
  if (value.type === 'list' && value.values.indexOf(payload) < 0) {
    ...
    if (this.discovered[vID]) {
      let map = this.discovered[vID].mode_map
...
Gateway.prototype.discoverDevice = function (node, hassDevice) {
  ...
  if (hassID && !node.hassDevices[hassID]) { // discover the device
    ...
    if (hassDevice.type === 'climate') {
      ...
      var setId = payload.temperature_state_topic[mode.value] ? payload.temperature_state_topic[mode.value] : payload.temperature_state_topic[hassDevice.mode_map.heat]
