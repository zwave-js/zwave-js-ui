...
function onNotification (node, notificationLabel, parameters) {
  const topic =
    this.nodeTopic(node) +
    '/notification/' +
    this.mqtt.cleanName(notificationLabel, true)
  let data
  ...
  if (topic.length === 0) {
    switch (this.config.type) {
      case 1: // named
        ...
        topic.push(this.mqtt.removeSlash(valueId.propertyName))
        if (valueId.propertyKey) {
          topic.push(this.mqtt.removeSlash(valueId.propertyKey))
        }
        break
      case 0: // valueid
        if (!this.config.nodeNames) {
        ...
        topic.push(this.mqtt.removeSlash(valueId.property))
        if (valueId.propertyKey) {
          topic.push(this.mqtt.removeSlash(valueId.propertyKey))
        }
        break
  ...
  try {
    if (hassID && !node.hassDevices[hassID]) {
      ...
      if (payload) {
        ...
        hassDevice.object_id = this.mqtt.cleanName(hassDevice.object_id, true)
...
Gateway.prototype.discoverValue = function (node, vId) {
  ...
  try {
      ...
      case CommandClasses['Binary Sensor']:
        ...
        if (sensorTypeName) {
          sensorTypeName = this.mqtt.cleanName(
            sensorTypeName.toLocaleLowerCase(), true
          )
        }
    ...
    cfg.object_id = this.mqtt.cleanName((cfg.object_id, true)
    ...
    // Set a unique id for the component
    payload.unique_id = 'zwavejs2mqtt_' + this.zwave.homeHex + '_' + this.mqtt.cleanName(valueId.id, true)
