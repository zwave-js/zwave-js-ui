...
/**
 *
 * @param {*} value configuration choice
 * @param {*} cfg device payload/cfg/hassdevice
 * @param {*} valueId node value info
 * @param {*} nodeName compiled nodeName
 */
function setHassFriendlyName (value, cfg, valueId, nodeName) {
  switch (value) {
    case 0:
      return `${nodeName.replace('-', ' ')} ${cfg.object_id.replace(/_/g, ' ')}`
    case 1:
      return cfg.object_id
    case 2:
      // Use PropertyKey, if not exists fall back to propertyName
      return valueId.propertyKey ? valueId.propertyKey : valueId.propertyName
    case 3:
      // Use PropertyKey, if not exists fall back to propertyName
      return `${nodeName} ${
        valueId.propertyKey ? valueId.propertyKey : valueId.propertyName
      }`
    case 4:
      // Use PropertyKey, if not exists fall back to propertyName
  ...
  try {
    if (hassID && !node.hassDevices[hassID]) {
      ...
      if (payload) {
        ...
        payload.name = setHassFriendlyName(
          this.config.hassFriendlyName,
          hassDevice,
          valueId,
          nodeName
        )
