  ...
  try {
    if (hassID && !node.hassDevices[hassID]) {
      ...
      if (payload) {
        ...
        switch (this.config.hassFriendlyName) {
          case 1:
            payload.name = `${node.loc} ${
              node.name
            } ${hassDevice.object_id.replace(/_/g, ' ')}`
            break
          case 2:
            ...
            break
          case 3:
            payload.name = `${hassDevice.object_id.replace(/_/g, ' ')}`
            break
          default:
...
Gateway.prototype.discoverValue = function (node, vId) {
  ...
  try {
    ...
    switch (this.config.hassFriendlyName) {
      case 1:
        payload.name = `${node.loc} ${node.name} ${cfg.object_id.replace(
          /_/g,
          ' '
        )}`
        break
      case 2:
        payload.name = `${node.name} ${node.loc} ${cfg.object_id.replace(
          /_/g,
          ' '
        )}`
        break
      case 3:
        payload.name = `${cfg.object_id.replace(/_/g, ' ')}`
        break
      default:
