  ...
  if(node.ready){ //enable poll if required
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      if(!this.zwave.client.isPolled(v))
        this.zwave.callApi('enablePoll', v, values[i].pollIntensity)
    }
  }
...
function onApiRequest(topic, apiName, payload){
  if(this.zwave){
    ...
    debug("The requested Zwave api", apiName, "doesn't exist")
  }
}
...
function onBroadRequest(parts, payload){
  if(this.config.type == 2){
    ...
    if(valueConf){
      payload = parsePayload(payload, valueConf.value, valueConf, this.config)
      this.zwave.writeBroadcast(valueConf.value, valueConf.device, payload);
    }
  }
}
