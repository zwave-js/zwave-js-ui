...
Gateway.prototype.parsePayload = function (payload, value, valueConf) {
  ...
  if (value.type === 'list' && value.values.indexOf(payload) < 0) {
    let vID = getValueID(value)
    var hassDevice = this.discovered[vID]
    if (hassDevice) {
      var map = hassDevice.mode_map
      if (map) {
        var node = this.zwave.nodes[value.node_id]
        // need to get the new value object
        value = node ? node.values[getValueID(value)] : value
        // need to update setpoint topic
        if (node && payload !== 'off' && map[payload] !== value.value && hassDevice.setpoint_topic) {
          var setID = hassDevice.setpoint_topic[map[payload]]
          let setpoint = node.values[setID]
          if (setpoint) {
            hassDevice.discovery_payload.temperature_state_topic = this.mqtt.getTopic(this.valueTopic(node, setpoint))
            hassDevice.discovery_payload.temperature_command_topic = hassDevice.discovery_payload.temperature_state_topic + '/set'
            this.mqtt.publish(hassDevice.discoveryTopic, hassDevice.discovery_payload, { qos: 0, retain: false })
          }
        }
        payload = map[payload]
