...
Gateway.prototype.parsePayload = function (payload, valueId, valueConf) {
  ...
  if (this.discovered[valueId.value_id]) {
    ...
    if ((valueId.type === 'bool' || this.discovered.object_id === 'rgb_dimmer') && typeof payload === 'string') {
      if (/\btrue\b|\bon\b/gi.test(payload)) payload = valueId.type === 'bool' ? true : valueId.max || 100
      else if (/\bfalse\b|\boff\b/gi.test(payload)) payload = valueId.type === 'bool' ? false : valueId.min || 0
    }
...
Gateway.prototype.publishDiscovery = function (hassDevice, nodeId, deleteDevice, update) {
  ...
  this.mqtt.publish(hassDevice.discoveryTopic, deleteDevice ? '' : hassDevice.discovery_payload, { qos: 0, retain: false }, this.config.discoveryPrefix)
  ...
  try {
    ...
    switch (type) {
      ...
      case 'switch_toggle_binary':
        let rgb = node.values.find(v => v.class_id === 0x33 && v.index === 0)
        if (rgb) {
          cfg = copy(hassCfg.light_rgb_switch)
          cfg.discovery_payload.rgb_state_topic = this.mqtt.getTopic(this.valueTopic(node, rgb, true))
      ...
      case 'switch_toggle_multilevel':
        ...
        if (valueId.index === 0) { // brightness level
          ...
          if (rgb) {
            ...
            cfg.discovery_payload.rgb_state_topic = this.mqtt.getTopic(this.valueTopic(node, rgb, true))
            cfg.discovery_payload.rgb_command_topic = cfg.discovery_payload.rgb_state_topic + '/set'
            cfg.brightness_state_topic = this.mqtt.getTopic(topic)
            cfg.brightness_command_topic = cfg.brightness_state_topic + '/set'
          } else {
