...
function addRgbColorSwitch (cfg, node, valueId) {
  ...
  if (node.values[`38-${valueId.endpoint}-currentValue`]) {
    brightnessValue = `38-${valueId.endpoint}-currentValue`
  } else if (node.values[`38-${valueId.endpoint + 1}-currentValue`]) {
    brightnessValue = `38-${valueId.endpoint + 1}-currentValue`
  } else if (node.values[`37-${valueId.endpoint}-currentValue`]) {
  ...
  */
  ...
  cfg.discovery_payload.white_value_scale = Object.keys(Constants.deviceClass.rgbWhiteTemp).length
