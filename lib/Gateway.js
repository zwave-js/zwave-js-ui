 ...
 * @param {*} valueId the valueId or CFG
 * @param {*} nodeName compiled nodeName
 * @param {*} valueId node value info
 */
function getEntityName (node, valueId) {
  const entityTemplate = this.config.entityTemplate
    ? this.config.entityTemplate
    : '%l-%n_%ob'
  return entityTemplate
    .replace(/%n/g, node.name)
    .replace(/%l/g, node.loc || '')
    .replace(/%pk/g, valueId.propertyKey || valueId.propertyName)
    .replace(/%pn/g, valueId.propertyName)
    .replace(/%ob/g, valueId.object_id)
    .replace(/%lb/g, valueId.label || valueId.object_id)
}
  ...
  try {
    if (hassID && !node.hassDevices[hassID]) {
      ...
      if (payload) {
        ...
        payload.name = getEntityName(node, hassDevice)
...
Gateway.prototype.discoverValue = function (node, vId) {
  ...
  try {
    ...
    // Set a friendly name for this component
    payload.name = getEntityName(node, valueId)
