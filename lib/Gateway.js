...
function getPriorityCCFirst (values) {
  let resultedList = {}
  // for (const priorityCommandClass of Constants.priorityCommandClass) {
  const filtered = values
    .filter(value => Constants.priorityCommandClass.includes(value.commandClass))
    .reduce((obj, key) => {
      obj[key] = values[key]
      return obj
    }, {})
  resultedList = filtered
  // }
  return resultedList
}
...
Gateway.prototype.rediscoverNode = function (nodeID) {
  ...
  if (node) {
    ...
    const nodeDevices = allDevices[node.deviceId] || []
    nodeDevices.forEach(device => this.discoverDevice(node, device))
    getPriorityCCFirst(node.values)
    // discover node values (that are not part of a device)
    for (const id in node.values) {
      this.discoverValue(node, id)
    }
