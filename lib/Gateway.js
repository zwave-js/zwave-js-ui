...
Gateway.prototype.discoverDevice = function (node, hassDevice) {
  ...
  try {
    if (hassID && !node.hassDevices[hassID]) { // discover the device
      ...
      } else if (hassDevice.type === 'fan') {
        payload = hassDevice.discovery_payload
        var fan = node.values[payload.state_topic]
        payload.state_topic = this.mqtt.getTopic(this.valueTopic(node, fan))
        payload.command_topic = payload.state_topic + '/set'
        payload.speed_command_topic = payload.command_topic
        payload.speed_state_topic = payload.state_topic
      }
    ...
    switch (type) {
      ...
      case 'door_lock':
        if (valueId.index === 0) { // lock state
          cfg = copy(hassCfg.binary_sensor_lock)
        } else {
          return
        }
        break
      ...
      case 'sensor_alarm':
        ...
        break
      case 'alarm':
        if (valueId.index === 0) cfg = copy(hassCfg.sensor_alarm_type)
        else if (valueId.index === 1) cfg = copy(hassCfg.sensor_alarm_level)
        else if (valueId.index === 10) cfg = copy(hassCfg.sensor_alarm_burglar)
        else return
        break
