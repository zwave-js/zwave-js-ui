 ...
 * @param {string} nodeName Node name from getNodeName function
 * @param {string} location Node location
 * @returns The topic string for this device discovery
 */
function getDiscoveryTopic (hassDevice, nodeName, location) {
  return `${hassDevice.type}/${utils.sanitizeTopic(nodeName + location ? '_' + location : '', true)}/${hassDevice.object_id}/config`
}
  ...
  try {
    if (hassID && !node.hassDevices[hassID]) {
      ...
      if (payload) {
        ...
        const discoveryTopic = getDiscoveryTopic(hassDevice, nodeName, node.loc)
...
Gateway.prototype.discoverValue = function (node, vId) {
  ...
  try {
    ...
    const discoveryTopic = getDiscoveryTopic(cfg, nodeName, node.loc)
