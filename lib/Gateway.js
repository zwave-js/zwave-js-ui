...
function addRgbColorSwitch (cfg, node, valueId) {
  ...
  if (node.values.hasOwnProperty(`38-${valueId.endpoint}-currentValue`)) {
    ...
    node.values.hasOwnProperty(`38-${valueId.endpoint + 1}-currentValue`)
  ) {
    brightnessValue = `38-${valueId.endpoint + 1}-currentValue`
  } else if (
    node.values.hasOwnProperty(`37-${valueId.endpoint}-currentValue`)
  ) {
  ...
  */
  if (brightnessValue) {
    ...
    cfg.discovery_payload.brightness_command_topic = this.mqtt.getTopic(
      this.valueTopic(node, node.values[brightnessValue], true).targetTopic
    ) + '/set'
  ...
  if (brightnessValue || switchValue) {
    cfg.discovery_payload.state_topic = cfg.discovery_payload.brightness_state_topic
    cfg.discovery_payload.command_topic = cfg.discovery_payload.brightness_command_topic
  }
