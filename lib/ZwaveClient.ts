...
class ZwaveClient extends EventEmitter {
  ...
  getGroups(nodeId: number, ignoreUpdate = false) {
    ...
    if (node && zwaveNode) {
      ...
      >;
      try {
        endpointGroups = this.driver.controller.getAllAssociationGroups(nodeId);
      } catch (error) {
        logger.warn(
  ...
  getAssociations(nodeId: number): Z2MGroupAssociation[] {
    ...
    if (zwaveNode) {
      try {
        ...
        const result = this.driver.controller.getAllAssociations(nodeId);
  ...
  async addAssociations(
    ...
    const sourceMsg = `Node ${
      source.nodeId + (source.endpoint ? " Endpoint " + source.endpoint : "")
    }`;
    ...
    if (zwaveNode) {
      try {
        const allAssociations =
          this.driver.controller.getAllAssociations(nodeId);
  ...
  async checkForConfigUpdates(): Promise<string | undefined> {
    if (this.driver && !this.closed) {
      this.driverInfo.newConfigVersion =
        await this.driver.checkForConfigUpdates();
  ...
  */
   ...
   */
  _getValueID(v: Partial<Z2MValueId> & { [x: string]: any }, withNode = false) {
