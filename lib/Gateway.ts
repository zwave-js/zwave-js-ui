...
export default class Gateway {
  ...
  valueTopic(
    ...
    if (returnObject && valueId.targetValue) {
      ...
      if (targetValue) {
        targetTopic = this.valueTopic(node, targetValue, false) as string;
      }
    }
        ...
        if (hassDevice.type === "climate") {
          ...
          for (let i = 0; i < hassDevice.values.length; i++) {
            ...
            topics[v] = node.values[v]
              ? this.mqtt.getTopic(
                  this.valueTopic(node, node.values[v]) as string
                )
              : null;
          }
   ...
   */
  _onValueChanged(valueId: Z2MValueId, node: Z2MNode, changed: boolean): void {
   ...
   */
  _getBinarySensorConfig(devClass: string, reversePayload = false): HassDevice {
  ...
  */
    ...
    if (brightnessValue || switchValue) {
      ...
      const topics = this.valueTopic(node, valueIdState, true) as ValueIdTopic;
