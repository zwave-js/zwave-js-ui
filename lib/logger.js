...
const sanitizedConfig = (module, config) => {
  ...
  return {
    ...
    filename:
      config.filename !== undefined ? config.filename : defaultLogFile
  }
}
...
const customFormat = (config, useColors) =>
  combine(
    ...
    printf(
      (info) => {
        info.timestamp = useColors ? colorizer.colorize('time', info.timestamp) : info.timestamp
        info.label = useColors ? colorizer.colorize('module', info.label || '-') : info.label
